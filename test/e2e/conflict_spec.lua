...
describe('Conflict', function()
  ...
  it('Opens 3 buffers when opening a file with git conflict', function()
    utils.open_file(nvim, './test/fixtures/conflict.txt')
  ...
  it('Remains within the original buffer if no conflicts found in a file', function()
    utils.open_file(nvim, './test/fixtures/no_conflict.txt')
    local bufCount = vim.fn.rpcrequest(nvim, 'nvim_eval', 'len(nvim_list_bufs())')
    eq(1, bufCount)
  end)
  ...
  it('Renders proper buffer content for longer left side', function()
    ...
    local lhs = vim.fn.rpcrequest(nvim, 'nvim_buf_get_lines', utils.LEFT_BUFFER, 0, -1, false)
    local rhs = vim.fn.rpcrequest(nvim, 'nvim_buf_get_lines', utils.RIGHT_BUFFER, 0, -1, false)
